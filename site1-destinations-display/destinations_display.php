    ...
    <section class="pkgd-section" id="<?php echo esc_attr( $uid ); ?>">
        ...
        <div class="pkgd-panel <?php echo $atts['default_tab'] === 'country' ? 'is-active' : ''; ?>" data-panel="country">
            <?php if ( ! empty( $countries ) ) :
                $c_pages = array_chunk( $countries, 4 );
                $c_dots  = count( $c_pages );
            ?>
                <div class="pkgd-carousel">
                    <div class="pkgd-carousel-track">
                        <?php foreach ( $c_pages as $page ) : ?>
                            <div class="pkgd-page">
                                <?php foreach ( $page as $c ) echo pkg_destinations_card( $c ); ?>
                            </div>
                        <?php endforeach; ?>
                    </div>
                    <?php if ( $c_dots > 1 ) : ?>
                    <div class="pkgd-dots">
                        <?php for ( $i = 0; $i < $c_dots; $i++ ) : ?>
                            <span class="pkgd-dot<?php echo $i === 0 ? ' is-active' : ''; ?>"></span>
                        <?php endfor; ?>
                    </div>
                    <?php endif; ?>
                </div>
        ...
        <?php if ( $atts['show_tabs'] === 'yes' ) : ?>
            <div class="pkgd-panel <?php echo $atts['default_tab'] === 'continent' ? 'is-active' : ''; ?>" data-panel="continent">
                <?php if ( ! empty( $continents ) ) :
                    $t_pages = array_chunk( $continents, 4 );
                    $t_dots  = count( $t_pages );
                ?>
                    <div class="pkgd-carousel">
                        <div class="pkgd-carousel-track">
                            <?php foreach ( $t_pages as $page ) : ?>
                                <div class="pkgd-page">
                                    <?php foreach ( $page as $c ) echo pkg_destinations_card( $c ); ?>
                                </div>
                            <?php endforeach; ?>
...
/* ── Carousel ── */
.pkgd-carousel { margin-bottom: 32px; }
.pkgd-carousel-track {
    display: grid;
...
@media (min-width: 601px) { .pkgd-page { display: contents; } }
@media (max-width: 900px) { .pkgd-carousel-track { grid-template-columns: repeat(2, 1fr); gap: 12px; } }
@media (max-width: 600px) {
    .pkgd-carousel { margin-bottom: 4px; }
    .pkgd-carousel-track {
    ...
    .pkgd-carousel-track::-webkit-scrollbar { display: none; }
    .pkgd-page {
        display: flex;
...
.pkgd-dot.is-active { background: #cc0000; transform: scale(1.3); }
@media (max-width: 600px) {
    .pkgd-dots { display: flex; justify-content: center; gap: 6px; padding: 10px 0 16px; }
}
...
.pkgd-card:hover .pkgd-card-arrow { color: #cc0000; transform: translateX(3px); }
...
@media (max-width: 600px) {
    ...
    .pkgd-card-flag { width: 46px; height: 46px; }
    .pkgd-card-name { font-size: 15px; }
    .pkgd-card-price { font-size: 12px; }
    .pkgd-section { padding: 36px 14px; }
    .pkgd-title { font-size: 24px; }
}
...
(function() {
    function init() {
        ...
        document.querySelectorAll('.pkgd-carousel').forEach(function(carousel) {
            var track = carousel.querySelector('.pkgd-carousel-track');
            var dots  = carousel.querySelectorAll('.pkgd-dot');
